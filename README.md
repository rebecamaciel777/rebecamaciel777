<h1>
<img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="30px">
  Bem-vindo ao meu portfólio! / Welcome to my portfolio!
</h1>

<img src="https://caccioppoli.com/Gif%20linee%20animate/Moving_squares.gif" width="100%">

# REBECA MACIEL
👩‍💻 **Desenvolvedora FULL STACK** / **FULL STACK Developer**

## Sobre Mim / About Me

**Português**:  
Desenvolvedora front end versátil e motivada, com foco na criação de soluções acessíveis e práticas. Habilidosa em desenvolvimento front-end, sou uma aprendiz rápida que adora resolver desafios e colaborar em ambientes dinâmicos.

**English**:  
Versatile and driven front end developer with a focus on creating accessible and practical solutions. Skilled in front-end development, I’m a quick learner who thrives on solving challenges and collaborating in dynamic environments.

<table>
  <tr>
    <td><strong>Habilidades</strong></td>
    <td><strong>Skills</strong></td>
  </tr>
  <tr>
    <td><ul>
      <li>Comunicação eficaz e trabalho em equipe.</li>
      <li>Organizada, adaptável e aprendiz rápida.</li>
      <li>Solução proativa de problemas com foco em acessibilidade.</li>
    </ul></td>
    <td><ul>
      <li>Effective communication and teamwork.</li>
      <li>Organized, adaptable, and quick learner.</li>
      <li>Proactive problem-solving and focus on accessibility.</li>
    </ul></td>
  </tr>
  <tr>
    <td><strong>Idiomas</strong></td>
    <td><strong>Languages</strong></td>
  </tr>
  <tr>
    <td><ul>
      <li>Português: Nativo.</li>
      <li>Inglês: Fluente.</li>
      <li>Espanhol: Intermediário.</li>
    </ul></td>
    <td><ul>
      <li>Portuguese: Native.</li>
      <li>English: Fluent.</li>
      <li>Spanish: Intermediate.</li>
    </ul></td>
  </tr>
</table>

## Tecnologias e Ferramentas / Technologies & Tools
<table>
  <tr>
    <th>Front End</th>
    
  </tr>
  <tr>
    <td>
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/html5/html5-original-wordmark.svg" width="50"/> 
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/css3/css3-original-wordmark.svg" width="50"/> 
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/javascript/javascript-original.svg" width="50"/> 
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/typescript/typescript-original.svg" width="50"/> 
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/react/react-original-wordmark.svg" width="50"/>
    </td>
  </tr>
</table>

<h2>
  <img src="https://cdn-icons-png.flaticon.com/512/614/614690.png" width="4%">
  Entre em Contato / Contact me</h2>
  </header>
  
  <div class="contact-container">
          <p><a href="mailto:rebecamacielr@outlook.com">rebecamacielr@outlook.com</a></p>
      </div>

          
          
          
          

          
          

                    

          

          
